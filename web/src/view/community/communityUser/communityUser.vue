    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="学历" prop="education" width="120">
          <template #default="scope">
            {{ filterDict(scope.row.education, educationOptions) }}
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="签名" prop="reamrk" width="120" />
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateCommunityUserFunc(scope.row)">变更</el-button>
...
const getTableData = async () => {
  const table = await getCommunityUserList({
    page: page.value,
...
const onDelete = async () => {
  ...
  const res = await deleteCommunityUserByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
// 更新行
const updateCommunityUserFunc = async (row) => {
  const res = await findCommunityUser({ ID: row.ID });
  type.value = "update";
  if (res.code === 0) {
...
// 删除行
const deleteCommunityUserFunc = async (row) => {
  const res = await deleteCommunityUser({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
